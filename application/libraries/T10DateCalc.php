<?
	class T10DateCalc{
		...
		var $_indoMonthShort = array('','Jan','Feb','Mar','Apr','Mei','Juni','Juli','Agust','Sep','Okt','Nov','Des');
 		
		/* Parameter pada konstruktor yang harus dimasukkan adalah sebuah tanggal 
		   dengan format 'Y-m-d' atau 'Y-m-d H:i:s'	*/
		function setT10DateCalc($dateString){
		...
		function getIndonesianFormat($now, $type=FALSE){
			// akan mengembalikan nilai 05 September 2008 apabila anda memasukan format tanggal 2008-09-05
			if($type == TRUE){
				return $this->_d .' '. $this->_indoMonthShort[date('n',$this->_time)] .' '. $this->_y;
			}else{
